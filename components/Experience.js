...
const typeConfig = {
  project: { icon: Star, color: 'bg-[#0B100D] text-primary-400 border border-[#1F2922]', dot: 'bg-primary-500' },
  work: { icon: Briefcase, color: 'bg-[#0B100D] text-primary-400 border border-[#1F2922]', dot: 'bg-primary-500' },
  education: { icon: GraduationCap, color: 'bg-[#0B100D] text-primary-400 border border-[#1F2922]', dot: 'bg-primary-500' },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center flex-shrink-0">
        ...
        {!isLast && (
          <div className="w-px flex-1 mt-2 bg-[#1F2922]" />
        )}
      </div>
      ...
      <div className="card p-5 flex-1 hover:-translate-y-0.5 mb-0">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div>
            <h3 className="font-bold text-[#F5F5F5]">{item.title}</h3>
            <p className="text-sm text-primary-400 font-medium">
              {item.company}
            </p>
          </div>
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-[#0B100D] border border-[#1F2922] text-[#A3A3A3] flex-shrink-0 font-mono">
            {item.year}
          </span>
        </div>
